...
describe('Todo API', () => {
    ...
    app.use(todo_1.default); // Mount tanpa prefix agar semua endpoint router bisa diakses langsung
    let token;
    let listId;
    let taskId;
    const testUser = {
    ...
    beforeAll(async () => {
        await (0, supertest_1.default)(app).post('/auth/register').send(testUser);
        const res = await (0, supertest_1.default)(app).post('/auth/login').send({ email: testUser.email, password: testUser.password });
    ...
    it('should get all lists for user', async () => {
        ...
        expect(res.body.some((l) => l.id === listId)).toBe(true);
    });
    ...
    it('should return 401 if no token', async () => {
        const res = await (0, supertest_1.default)(app).get('/lists');
    ...
    it('should get all tasks in a list', async () => {
        ...
        const listsRes = await (0, supertest_1.default)(app)
            .get('/lists')
            .set('Authorization', `Bearer ${token}`);
        const list = listsRes.body.find((l) => l.name === 'Task List');
        const res = await (0, supertest_1.default)(app)
    ...
    it('should return 404 if update task not owned by user', async () => {
        ...
        const resLogin = await (0, supertest_1.default)(app)
            .post('/auth/login')
