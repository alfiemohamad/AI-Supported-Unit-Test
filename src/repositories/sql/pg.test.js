...
describe('pg.ts', () => {
    it('should connect to PostgreSQL and run a simple query', async () => {
        await expect((0, pg_1.testConnection)()).resolves.toBeUndefined();
    });
    it('should be able to run a query via pool', async () => {
        const res = await pg_1.pool.query('SELECT 1 as result');
        expect(res.rows[0].result).toBe(1);
    });
    it('should throw error if query is invalid', async () => {
        await expect(pg_1.pool.query('SELECT * FROM not_a_table')).rejects.toThrow();
    });
    /*
